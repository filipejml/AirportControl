...
<div class="container mt-4">
    ...
    <div class="card shadow-sm">
        <div class="card-body p-4">
            <form method="POST" action="{{ route('aeroportos.update', $aeroporto) }}" id="aeroportoForm">
                @csrf
                @method('PUT')
                        ...
                        <div class="row g-4" id="companhiasContainer">
                            @foreach($companhias as $companhia)
                                <div class="col-md-4 col-lg-3">
                                    <div class="companhia-card {{ in_array($companhia->id, old('companhias', $aeroporto->companhias->pluck('id')->toArray())) ? 'selected' : '' }}" 
...
let currentAirportId = {{ $aeroporto->id }};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nomeInput) {
        // Verificar nome atual (se já existe) - pular validação para o nome atual
        const currentNome = nomeInput.value.trim();
        if (currentNome) {
            // Marcar como válido inicialmente para edição
            isNameValid = true;
            nomeInput.classList.add('is-valid');
            const checkIcon = document.getElementById('nomeCheckIcon');
        ...
        nomeInput.addEventListener('input', function(e) {
            ...
            if (nome === '') {
                ...
                // Passar o ID do aeroporto atual para verificar se é o mesmo
                checkAirportName(nome, currentAirportId);
            }
        });
